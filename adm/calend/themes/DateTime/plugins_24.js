...
var _timeFormat=new RegExp("^([0-1]?[0-9]|2[0-3]):[0-5]?[0-9]$");
function time2str(hour, minute, ampm) { // format time in "HH:MI24" and rounded according to interval
	return padZero(hour)+":"+padZero(Math.floor(minute/_inc)*_inc);
}

function formatTime(str) {
	if (_timeFormat.test(str)==false) { // use current time if str is invalid
		var nd=new Date();
		return time2str(nd.getHours(),nd.getMinutes());
	} else
		return str;
}
...
function updateTimeStr() {
	...
	_timeVal=time2str(bf.hourF.value,bf.minF.value);
