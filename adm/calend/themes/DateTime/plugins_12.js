...
function fOnResize() {
	...
	bf.hourF.value=t[0];
	bf.minF.value=t[1].substring(0,2);
	bf.ampm.value=t[1].substring(t[1].length-2,t[1].length);
}
...
function fParseInput(str) {
	...
	_timeVal=formatTime(dt[1]+(dt[2]?" "+dt[2]:""));
...
gsBottom=(NN4?'':'<DIV align="center" class="BottomDiv">')+'<table border="0" cellpadding="0" cellspacing="0" width="1"><tr><td><input type="text" name="hourF" size="2" maxlength="2" class="TimeBox" onchange="updateTimeStr()" onfocus="this.value=\'\'"></td><td><img border="0" src="plus.gif" onmousedown="incHour();" onmouseup="stopTime()" width="13" height="9"><br><img border="0" src="minus.gif" onmousedown="decHour();" onmouseup="stopTime()" width="13" height="9"></td><td nowrap>&nbsp;<B>:</B>&nbsp;</td><td><input type="text" name="minF" size="2" maxlength="2" class="TimeBox" onchange="updateTimeStr()" onfocus="this.value=\'\'"></td><td><img border="0" src="plus.gif" onmousedown="incMin();" onmouseup="stopTime()" width="13" height="9"><br><img border="0" src="minus.gif" onmousedown="decMin();" onmouseup="stopTime()" width="13" height="9"></td><td>&nbsp;</td><td><input type="Text" name="ampm" size="2" maxlength="2" class="TimeBox" readonly onmousedown="flipAmPm()" ></td></tr></table>'+(NN4?'':'</DIV>');
...
var _timeFormat=new RegExp("^([0]?[1-9]|1[0-2]):[0-5]?[0-9] ("+_AM+"|"+_PM+")$");
function time2str(hour, minute, ampm) { // format time in "HH:MI AM" and rounded according to interval
	return padZero(hour)+":"+padZero(Math.floor(minute/_inc)*_inc)+" "+ampm;
}

function formatTime(str) {
	if (_timeFormat.test(str)==false) { // use current time if str is invalid
		var nd=new Date(), h=nd.getHours();
		if (h>12||h==0) h=Math.abs(h-12);
		return time2str(h,nd.getMinutes(),h>11?_PM:_AM);
	} else
		return str;
}
...
function updateTimeStr() {
	var bf=document.cxpBottomForm;
	bf.hourF.value=/^([0]?[1-9]|1[0-2])$/.test(bf.hourF.value)?padZero(bf.hourF.value):"01";
	bf.minF.value=/^[0-5]?[0-9]$/.test(bf.minF.value)?padZero(Math.floor(parseInt(bf.minF.value,10)/_inc)*_inc):"00";
	_timeVal=time2str(bf.hourF.value,bf.minF.value,bf.ampm.value);
...
function flipAmPm() {
	var bf=document.cxpBottomForm;
	bf.ampm.value=bf.ampm.value=="AM"?"PM":"AM";
	updateTimeStr();
}
