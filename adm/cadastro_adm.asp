...
<head>
	...
	<%
	RS.open = "SELECT * FROM TURMAS WHERE ID_TURMA="&request("cod")
	COD_CURSO = RS("COD_CURSO")
	COD_TIPO = RS("COD_TIPO")
	CODIGO_INSCRICAO = RS("CODIGO_INSCRICAO")
	RS.Close
	RS.open = "SELECT * FROM CURSO WHERE COD_CURSO="&COD_CURSO
	BOLETO = RS("BOLETO")
    ...
    ' Verificação se foi chamado de dentro do SIGA - parametro SIGA=siga
    siga = ""
    siga = request("siga")

	cod = request("cod")
	if cod=0 or cod="" or isNumeric(cod)=false then cod=0
	RS.Open "SELECT C.TITULO,C.VALOR,C.SENHA,M.COD_CURSO FROM CURSO C JOIN TURMAS M ON C.COD_CURSO = M.COD_CURSO WHERE COD_STATUS=1 AND M.ID_TURMA="&int(cod)
	if RS.EOF then
	...
	if len(request("coda"))>0 then
		jaCadastrado = true
		sql = "SELECT * FROM ALUNO WHERE COD_ALUNO="&request("coda")
		...
		RS.Open "SELECT COUNT(*) FROM TURMA_ALUNO WHERE COD_ALUNO="&request("coda")&" AND COD_CURSO="&cod
		if RS(0)>0 then
			jaNoCurso = true
		else
			jaNoCurso = false
		...
		if jaNoCurso then
			...
			msql = "SELECT T.*,A.TIPOENTI,A.COD_UF_ENTI,A.COD_MUNI_ENTI,A.SETOR,A.CARGO,A.E_NOME,A.E_CNPJ,A.E_ENDERECO,A.E_CEP,A.E_EMAIL,A.E_WWW,A.E_TELEFONE,A.E_FAX FROM TURMA_ALUNO T JOIN ALUNO A ON T.COD_ALUNO = A.COD_ALUNO WHERE A.COD_ALUNO="&request("coda")&" AND COD_CURSO="&cod
		else
			'Selecione o último cadastro profissional

			msql = "SELECT TOP 1 T.*,A.TIPOENTI,A.COD_UF_ENTI,A.COD_MUNI_ENTI,A.SETOR,A.CARGO,A.E_NOME,A.E_CNPJ,A.E_ENDERECO,A.E_CEP,A.E_EMAIL,A.E_WWW,A.E_TELEFONE,A.E_FAX FROM TURMA_ALUNO T JOIN ALUNO A ON T.COD_ALUNO = A.COD_ALUNO WHERE T.COD_ALUNO="&request("coda")&" ORDER BY DT_CADASTRO DESC"
		end if
...
<div id="pagina">
	<!-- #INCLUDE FILE="../include/topo_adm.asp" -->
	<div id="conteudo">
	<h1>FORMULÁRIO DE INSCRIÇÃO</h1><p><br /></p>
	<h2>Curso: <%=TITULO%></h2>
	<p><br /></p>
	<%if precisaSenha and not Session("pwd"&cod)="ok" then%>
		...
		<input type="hidden" name="COD_ALUNO" value="<%=request("coda")%>">
	...
	<%else%>
		<%if not jaCadastrado then%>
			...
			<form action="login_adm.asp" method="post" name="cpflogin">
		    <input type="hidden" name="siga" value="<%=request("siga")%>">
			<input type="hidden" name="cod" value="<%=cod%>">
			<fieldset>
				...
				<p><label for="ENVIAR">&nbsp;</label><input type="submit" name="submit" value="Recuperar dados" class="buttonF"></p>
			</fieldset>
			</form>
		<%end if%>
		<%if jaNoCurso then%>
		...
		<%end if%>
		<form name="form" method="POST" action="cadastro_resp_adm.asp" name="form">
		<input type="hidden" name="siga" value="<%=request("siga")%>">
		<input type="hidden" name="cod_aluno" value="<%=request("coda")%>">
		<input type="hidden" name="cod" value="<%=cod%>">
		...
		<fieldset>
			...
			<div id="buscaEnti" style="display:block;">                                                                                                               <!---- WHERE DESCRICAO<>'Pessoa Física' ---->
				<p><label class="requerido" for="tipoEnti">Tipo de instituição</label><%=createSelSIM("tipoEnti","SELECT ID_TIPO_ENTIDADE,DESCRICAO FROM TIPO_ENTIDADE ",tipoEnti,1,"getEnti()")%></p>
				...
				<p><label for="E_NOME" class="requerido">Nome da Instituição</label><input type="text" name="E_NOME" id="E_NOME" maxlength="200" size="60" value="<%=E_NOME%>"></p>
				<p><label for="E_CNPJ" class="requerido">CNPJ</label><input type="text" name="E_CNPJ" id="E_CNPJ" maxlength="14" size="15" value="<%=E_CNPJ%>" onKeyPress="return ValidateKeyPressed('INT', this, event);"> somente números</p>
				<p><label for="E_ENDERECO" class="requerido">Endereço</label><input type="text" name="E_ENDERECO" id="E_ENDERECO" value="<%=E_ENDERECO%>" maxlength="200" size="60"></p>
				<p><label for="E_CEP" class="requerido">CEP</label><input type="text" name="E_CEP" id="E_CEP" maxlength="8" size="10" value="<%=E_CEP%>" onKeyPress="return ValidateKeyPressed('INT', this, event);"> somente números</p>
				<p><label for="E_EMAIL" class="requerido">E-mail</label><input type="text" name="E_EMAIL" id="E_EMAIL" maxlength="100" size="60" value="<%=E_EMAIL%>"></p>
				<p><label for="E_WWW">Site</label><input type="text" name="E_WWW" id="E_WWW" maxlength="100" size="60" value="<%=E_WWW%>"></p>
				<p><label for="E_TELEFONE" >Telefone / Ramal</label><input type="text" name="E_TELEFONE" id="E_TELEFONE" value="<%=E_TELEFONE%>" maxlength="9" size="20" onKeyPress="return ValidateKeyPressed('INT', this, event);"> / <input type="text" name="E_TELRM" id="E_TELRM" maxlength="4" size="5" onKeyPress="return ValidateKeyPressed('INT', this, event);"> somente números</p>
		...
		<fieldset>
			...
			<p><label class="requerido" for="FORMA_PAGAMENTO">Forma de pagamento</label>
			<select name="FORMA_PAGAMENTO">
				<option value=0></option>"
				<%IF BOLETO = 1 THEN%><option value=1>BOLETO</option><%END IF%>
				<%IF EMPENHO = 1 THEN%><option value=2>EMPENHO</option><%END IF%>
				<%IF PAGSEGURO = 1 THEN%><option value=3>PAG-SEGURO</option><%END IF%>
				<%IF DEPOSITO = 1 THEN%><option value=4>DEPÓSITO</option><%END IF%>
			</select></p>
			<p><label class="requerido" for="ALUNO_PARCELAS">Quantidade de parcelas</label>
			<select name="ALUNO_PARCELAS">
				<option value=0></option>"
				<%
					for i=1 TO PARCELAS
		...
		<% if COD_TIPO=4 then %>
			<fieldset>
				<p><label for="CODIGO_INSCRICAO" class="requerido">Código de Inscrição</label><input type="text" name="CODIGO_INSCRICAO" id="CODIGO_INSCRICAO" value="" maxlength="6" size="6"></p>
			</fieldset>
		<% end if %>
