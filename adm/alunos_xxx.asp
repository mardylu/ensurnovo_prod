		...
		if task<>"" then
			...
			select case task
				...
				case "turma":
					...
					%>
						<script language="javascript">
							window.location = 'alunos_xxx.asp?ord=<%=ord%>&cod=<%=cod%>&st=<%=st%>&formulario=turma';
						</script>
					<%
					end if
				case "nome":
					...
					%>
						<script language="javascript">
							window.location = 'alunos_xxx.asp?ord=<%=ord%>&cod=<%=cod%>&st=<%=st%>&formulario=nome';
						</script>
					<%
					end if
				case "lista":
					...
					%>
						<script language="javascript">
							window.location = 'alunos_xxx.asp?ord=<%=ord%>&cod=<%=cod%>&st=<%=st%>&formulario=lista';
						</script>
					<%
					end if
				case "frequencia":
				...
				case "enturmar":
					...
					RS.close
					sql = "SELECT A.CPF,A.PASSWORD,A.EMAIL,A.NOME,A.COD_ALUNO,A.COD_ENTI,A.TIPOENTI FROM ALUNO A JOIN TURMA_ALUNO TA ON A.COD_ALUNO=TA.COD_ALUNO WHERE A.COD_ALUNO IN ("&sel&")"
					RS.open sql
					while not RS.eof
						a = Split(RS("NOME"))
						i=0
...
'							pdf.SetPath("./fpdf/")
							...
							pdf.SetY(0)
							pdf.SetX(0)
							pdf.Image "certificadofrente.jpg",0,0,300,210,"JPG"
...
pdf.Close()
							...
							pdf.Output(path_certificado&"Certificado_"&RS("COD_ALUNO")&"_"&RS("COD_CURSO")&"_"&RS("ID_TURMA")&"A.pdf")
...
pdf.Open()
							...
							pdf.SetY(0)
							pdf.SetX(0)
							pdf.Image "certificadoverso.jpg",0,0,300,210,"JPG"
							pdf.SetFont "Arial","",11
							pdf.SetY(20)
							pdf.SetX(40)
...
pdf.Cell 40,4, "Riferimento" ,1,0
							...
							pdf.Close()
							arquivo=replace(RS("NOME")," ","_")
							pdf.Output(path_certificado&"Certificado_"&RS("COD_ALUNO")&"_"&RS("COD_CURSO")&"_"&RS("ID_TURMA")&"B.pdf")

pdf_frente ="C:\inetpub\wwwroot\ensurnovo\adm\certificados\Certificado_"&RS("COD_ALUNO")&"_"&RS("COD_CURSO")&"_"&RS("ID_TURMA")&"A.pdf"
' pdf_frente ="C:\inetpub\wwwroot\ensurnovo\adm\certificados\xxx_"&RS("COD_ALUNO")&"_"&RS("COD_CURSO")&"_"&RS("ID_TURMA")&".pdf"
...
set fs=nothing
						...
						<script language="javascript">
							...
							window.location = 'alunos_xxx.asp?ord=<%=ord%>&cod=<%=cod%>&st=<%=st%>';
						</script>
						<%
		...
		<div align="right">
		<form name="form" action="alunos_xxx.asp" method="get">
		...
		<form name="formAlunos" action="alunos_xxx.asp" method="post" onSubmit="return checkForm();">
		<input type="hidden" name="ord" value="<%=ord%>">
		<input type="hidden" name="cod" value="<%=cod%>">
...
'		response.write sql
		sql = "SELECT C.ENTURMADO,A.COD_ALUNO,A.COD_ENTI,A.TIPOENTI,A.COD_UF_IBGE,A.COD_MUNI_IBGE,T.COD_STATUS_TURMA,EMAIL_ENVIADO,STATUS,NOTA,TEL,TEL_DDD,NOME,CPF,APROVADO,DT_CADASTRO,PAGO,FORMA_PGT,A.EMAIL,A.E_NOME,A.CARGO,C.ID_TURMA,(SELECT SUM(VALOR) FROM PAGAMENTO WHERE COD_ALUNO=A.COD_ALUNO AND ID_TURMA=C.ID_TURMA AND DT_PGT IS NOT NULL) AS VALOR FROM TURMA_ALUNO C JOIN ALUNO A ON C.COD_ALUNO=A.COD_ALUNO JOIN TURMAS T ON T.ID_TURMA=C.ID_TURMA WHERE C.ID_TURMA="&cod&sqlwhere&" ORDER BY "&sqlord
		RS.Open sql
		if RS.EOF then
            ...
            <%  if (SIGA_PROJETO<>2)  then  %>
    				<td class="header"><a href="alunos_xxx.asp?ord=nm&cod=<%=cod%>&st=<%=st%>">nome</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=cp&cod=<%=cod%>&st=<%=st%>">cpf</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=cp&cod=<%=cod%>&st=<%=st%>">Telefone</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=fp&cod=<%=cod%>&st=<%=st%>">forma pgt.</a></td>
    				<%if valor>0 then%><td class="header"><a href="alunos_xxx.asp?ord=pg&cod=<%=cod%>&st=<%=st%>">pago</a>
    				<%end if%><td class="header"><a href="alunos_xxx.asp?ord=ap&cod=<%=cod%>&st=<%=st%>">Status.</a></td>
    				<td class="header">Ent.</td>
				    <td class="header"><a href="alunos_xxx.asp?ord=dc&cod=<%=cod%>&st=<%=st%>">dt. inscrição</a></td>
            <%  end if  %>

            <%  if (SIGA_PROJETO=2)  then    %>
    				<td class="header"><a href="alunos_xxx.asp?ord=nm&cod=<%=cod%>&st=<%=st%>">nome</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=cp&cod=<%=cod%>&st=<%=st%>">cpf</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=cp&cod=<%=cod%>&st=<%=st%>">Tipo Instituição</a></td>
<!----				<td class="header"><a href="alunos_xxx.asp?ord=cp&cod=<%=cod%>&st=<%=st%>">Nome Instituição</a></td> ---->
    				<td class="header"><a href="alunos_xxx.asp?ord=fp&cod=<%=cod%>&st=<%=st%>">UF</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=ap&cod=<%=cod%>&st=<%=st%>">Município</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=ap&cod=<%=cod%>&st=<%=st%>">Bioma</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=ap&cod=<%=cod%>&st=<%=st%>">Adesão</a></td>
    				<td class="header"><a href="alunos_xxx.asp?ord=ap&cod=<%=cod%>&st=<%=st%>">Status</a></td>
    				<td class="header">Ent</td>
				    <td class="header"><a href="alunos_xxx.asp?ord=dc&cod=<%=cod%>&st=<%=st%>">dt. inscrição</a></td>
            <%  end if    %>
				...
				<% else %>
					<td class="header"><a href="alunos_xxx.asp?ord=dc&cod=<%=cod%>&st=<%=st%>">Obs.</a></td>
					<td class="header"><a href="alunos_xxx.asp?ord=dc&cod=<%=cod%>&st=<%=st%>">e-mail.</a></td>
				<% end if %>
				</tr>
				<%
				c = 0
...
' Response.Write(sql)
                                	RS2.Open sql
                                	if not RS2.eof then
                                        BIOMA_MUNI_ALUNO="Sim"
                                        ADESAO_MUNICIPIO=RS2("ADESAO")
                                    else
                                		BIOMA_MUNI_ALUNO="Não"
                                        ADESAO_MUNICIPIO="Não"
                                	end if
                                	RS2.Close
			...
			<% elseif request("formulario")="lista" then%>
				...
				<td class="header"><a href="alunos_xxx.asp?ord=nm&cod=<%=cod%>&st=<%=st%>">Nome</a></td>
				<td class="header"><a href="alunos_xxx.asp?ord=cp&cod=<%=cod%>&st=<%=st%>">Telefone</a></td>
				<td class="header"><a href="alunos_xxx.asp?ord=fp&cod=<%=cod%>&st=<%=st%>">E-mail</a></td>
				<td class="header"><a href="alunos_xxx.asp?ord=dc&cod=<%=cod%>&st=<%=st%>">Entidade</a></td>
				<td class="header"><a href="alunos_xxx.asp?ord=dc&cod=<%=cod%>&st=<%=st%>">Função</a></td>
				</tr>
				<%
				c = 0
				while not RS.EOF
			...
			<% elseif request("formulario")="frequencia" then%>
				...
				<td class="header"><a href="alunos_xxx.asp?ord=nm&cod=<%=cod%>&st=<%=st%>">Nome</a></td>
				<td class="header"><a href="alunos_xxx.asp?ord=cp&cod=<%=cod%>&st=<%=st%>">CPF</a></td>
				<td class="header"><a href="alunos_xxx.asp?ord=fp&cod=<%=cod%>&st=<%=st%>">Manhã</a></td>
				<td class="header"><a href="alunos_xxx.asp?ord=fp&cod=<%=cod%>&st=<%=st%>"><br></a></td>
				<td class="header"><a href="alunos_xxx.asp?ord=dc&cod=<%=cod%>&st=<%=st%>">Tarde</a></td>
				</tr>
				<%
				c = 0
				while not RS.EOF
