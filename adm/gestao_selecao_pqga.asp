...
<div id="pagina">
	...
	<div id="conteudo">
		...
		select case st
			...
			case 3:
				tit = "Ensino fundamental incompleto"
				sqlwhere = " AND COD_ESCOLARIDADE='1'"
			case 4:
				tit = "Ensino fundamental completo"
				sqlwhere = " AND COD_ESCOLARIDADE='2'"
			case 5:
				tit = "Ensino médio incompleto"
				sqlwhere = " AND COD_ESCOLARIDADE='3'"
			case 6:
				tit = "Ensino médio completo"
				sqlwhere = " AND COD_ESCOLARIDADE='4'"
			case 7:
				tit = "Profissional de nível técnico incompleto"
				sqlwhere = " AND COD_ESCOLARIDADE='5'"
			case 8:
				tit = "Profissional de nível técnico completo"
				sqlwhere = " AND COD_ESCOLARIDADE='6'"
			case 9:
				tit = "Ensino superior incompleto"
				sqlwhere = " AND COD_ESCOLARIDADE='7'"
			case 10:
				tit = "Ensino superior completo"
				sqlwhere = " AND COD_ESCOLARIDADE='8'"
			case 11:
				tit = "TODOS"
				sqlwhere = ""
		end select
		...
		select case ord
			...
			case "ef": sqlord = "EFETIVO"                          ' servidor efetivo sim ou não
...
<% ' Response.Write(SIGA_PROJETO) %>
		...
		<select name="st" onChange="document.form.submit();">
			<option value="11"<%if st=11  then response.write " SELECTED"%>>Todos</option>
...
<!----
			<option value="3"<%if  st=3   then response.write " SELECTED"%>>Ensino fundamental incompleto</option>
			<option value="4"<%if  st=4   then response.write " SELECTED"%>>Ensino fundamental completo</option>
			<option value="5"<%if  st=5   then response.write " SELECTED"%>>Ensino médio incompleto</option>
			<option value="6"<%if  st=6   then response.write " SELECTED"%>>Ensino médio completo</option>
			<option value="7"<%if  st=7   then response.write " SELECTED"%>>Profissional de nível técnico incompleto</option>
			<option value="8"<%if  st=8   then response.write " SELECTED"%>>Profissional de nível técnico completo</option>
			<option value="9"<%if  st=9   then response.write " SELECTED"%>>Ensino superior incompleto</option>
			<option value="10"<%if st=10  then response.write " SELECTED"%>>Ensino superior completo</option>
---->
		</select>
		<br />
        ...
        sql_filtro  = ""
        if ( tipo_enti <> "0" )  then    sql_filtro = sql_filtro & " AND TIPOENTI='"          & tipo_enti & "'"   end if
        if ( escolar <> "0" )    then    sql_filtro = sql_filtro & " AND COD_ESCOLARIDADE='"  & escolar   & "'"   end if
        if ( termo <> "9" )      then    sql_filtro = sql_filtro & " AND TERMO_COMPROMISSO='" & termo     & "'"   end if
        if (bioma = 0) then sql_filtro = sql_filtro & " AND A.COD_UF_IBGE=B.COD_UF_IBGE  AND A.COD_MUNI_IBGE=B.COD_MUNI_IBGE "    end if
        if (bioma = 1) then sql_filtro = sql_filtro & " AND A.COD_UF_IBGE<>B.COD_UF_IBGE AND A.COD_MUNI_IBGE<>B.COD_MUNI_IBGE "   end if
        if ( servico <> "xxx" )  then    sql_filtro = sql_filtro & " AND EFETIVO='"           & servico   & "'"   end if

' Response.Write("<br>" & "entiTipo: " & tipo_enti)
' Response.Write("<br>" & "escolar: " & escolar)
' Response.Write("<br>" & "termo: " & termo)
' Response.Write("<br>" & "bioma: " & bioma)
' Response.Write("<br>" & "servico: " & servico & "<br>")



		sql =   "SELECT C.ENTURMADO,A.COD_ALUNO,A.COD_ESCOLARIDADE,A.EFETIVO,A.COD_ENTI,A.TIPOENTI,A.COD_UF_IBGE,"          & _
                "       A.COD_MUNI_IBGE,T.COD_STATUS_TURMA,C.TERMO_COMPROMISSO,EMAIL_ENVIADO,STATUS,NOTA,TEL,"              & _
                "       TEL_DDD,NOME,A.CPF,APROVADO,DT_CADASTRO,PAGO,FORMA_PGT,A.EMAIL,A.E_NOME,A.CARGO,"                   & _
                "       C.ID_TURMA,(SELECT SUM(VALOR) FROM PAGAMENTO WHERE COD_ALUNO=A.COD_ALUNO "                          & _
                "       AND ID_TURMA=C.ID_TURMA AND DT_PGT IS NOT NULL) AS VALOR FROM TURMA_ALUNO C "                       & _
                "       JOIN ALUNO  A ON C.COD_ALUNO=A.COD_ALUNO "                                                          & _
                "       JOIN TURMAS T ON T.ID_TURMA=C.ID_TURMA "

        if (bioma <> "9") then sql = sql & "  JOIN BIOMA_CIDADE B ON A.COD_UF_IBGE=B.COD_UF_IBGE  AND A.COD_MUNI_IBGE=B.COD_MUNI_IBGE "    end if

        sql = sql & " WHERE C.ID_TURMA=" & cod & sqlwhere  & sql_filtro
...
' Response.End
		RS.Open sql
		if RS.EOF then
			%><br /><p><br><b>Nenhum aluno cadastrado</b><br></p><%
		else
...
<!----    			<td class="header"><a href="gestao_selecao.asp?ord=cp&cod=<%=cod%>&st=<%=st%>">cpf</a></td> ---->
    				...
    				<td class="header"><a href="gestao_selecao.asp?ord=es&cod=<%=cod%>&st=<%=st%>">Escolaridade</a></td>
    	   			...
    	   			<td class="header"><a href="gestao_selecao.asp?ord=qt&cod=<%=cod%>&st=<%=st%>" title="Total de enturmações">TE&nbsp;</a></td>
    	   			<td class="header"><a href="gestao_selecao.asp?ord=qt&cod=<%=cod%>&st=<%=st%>" title="Total de participação em turmas finalizadas">TF&nbsp;</a></td>
...
<!----    			<td class="header"><a href="gestao_selecao.asp?ord=ad&cod=<%=cod%>&st=<%=st%>">Ads&nbsp;</a></td> ---->
...
<!----  TA  Total de aprovações do aluno    ---->
            				<td><%
                                	sql = "SELECT COUNT (a.COD_ALUNO) AS QTDE FROM TURMA_ALUNO  as a"   _
                                        & " LEFT JOIN CURSO as b ON a.cod_curso=b.cod_curso "   _
                                        & " WHERE b.SIGA_PROJETO=2 AND a.COD_ALUNO="&RS("COD_ALUNO")    _
                                        & " AND a.enturmado='E'    AND a.status='APROVADO' "
' Response.Write(sql)
' Response.End
                                	RS3.Open sql
                                	if not RS3.eof then
